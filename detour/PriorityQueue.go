...
func (s *sorter) GetIndex(v Interface) int {
	for i := 0; i < len(*s); i++ {
		if (*s)[i] == v {
			return i
		}
...
func New_PriorityQueue() *PriorityQueue {
...
func (q *PriorityQueue) Remove(v Interface) Interface {
	return heap.Remove(q.s, q.s.GetIndex(v)).(Interface)
}
